/* 场景生成页 — 结构树 + 表单编辑 + JSON/YAML 预览 + 实时校验（后端权威存储） */
(function () {
  "use strict";
  ...
  function markSaved() {
    var d = new Date();
    $("autosave-dot").className = "dot ok";
    $("autosave-text").textContent = "已自动保存 " + d.toTimeString().slice(0, 8);
  }

  function syncToServer() {
    $("autosave-dot").className = "dot warn";
    $("autosave-text").textContent = "正在同步后端…";
    ScenarioStore.push(S, function (err) {
      if (err) {
        $("autosave-dot").className = "dot danger";
        $("autosave-text").textContent = "后端同步失败（已本地缓存）";
      } else {
        $("autosave-dot").className = "dot ok";
        $("autosave-text").textContent = "已同步后端 " + new Date().toTimeString().slice(0, 8);
      }
    });
  }
  ...
  $("import-file").onchange = function () {
    ...
    var isYaml = /\.ya?ml$/i.test(f.name);
    r.onload = function () {
      var text = r.result;
      if (!isYaml) {
        try {
      ...
      // YAML 经后端解析与校验
      SCAPI.post("/api/scenario/import", { text: text, fmt: "yaml" }).then(function () {
        return ScenarioStore.pull();
        ...
        if (e && e.detail && e.detail.errors && e.detail.errors.length) {
          msg += "：" + e.detail.errors[0].loc + " " + e.detail.errors[0].msg;
        }
        scToast(msg, "danger");
      });
    };
  ...
  $("tb-export-yaml").onclick = function () {
    var fname = (S.meta.name || "场景") + "_v" + (S.meta.version || "1") + ".yaml";
    SCAPI.put("/api/scenario", S).then(function () {
      return fetch("/api/scenario/export?fmt=yaml");
    }).then(function (resp) { return resp.text(); }).then(function (text) {
      download(fname, text, "text/yaml");
      scToast("已导出 YAML");
    }).catch(function () {
      download(fname, ScenarioStore.toYAML(S), "text/yaml");
      scToast("后端不可用，已导出本地序列化 YAML", "warn");
    });
  };
  ...
  ScenarioStore.pull().then(function (remote) {
    ...
    scToast("后端未连接，当前为本地缓存编辑模式", "warn");
  });
})();
